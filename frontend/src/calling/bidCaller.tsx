export function getBid(id: number, user: { token: string }): Promise<any> {
  ...
  export function getReceivedBids(userId: number, user: { token: string }): Promise<any[]> {
    ...
    .then(response => {
      if (response.status === 200) {
        ...
        throw new Error(`Failed to fetch received bids: ${response.statusText}`);
      }
    })
    .catch(err => {
      console.error("Error:", err);
  ...
  export function getOfferedBids(userId: number, user: { token: string }): Promise<any[]> {
  ...
  export function offerBid(bidDto: any, user: { token: string }): Promise<void> {
