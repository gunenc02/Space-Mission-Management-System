...
export default function Platforms() {
  ...
  return (
      <div className="outer flex flex-col items-center bg-blue-100 min-h-screen">
        ...
        <div className="w-full max-w-6xl p-6 overflow-auto" style={{ maxHeight: 'calc(100vh - 200px)' }}> {/* Adjust 100px based on the combined height of Header and Navbar */}
            ...
            {filteredPlatforms.map((platform: Platform) => (
                <Link to={"/platform/" + platform.id}
                  ...
                  <div className="w-full md:w-1/3 h-48">
                    <img src={platform.image} alt={platform.platformName} className="w-full h-full object-cover"/>
                  </div>
                  <div className="p-4">
        ...
        {isFilterModalOpen && <PlatformsFilter onClose={() => setIsFilterModalOpen(false)} onFilter={handleFilter} />}
        {isAddPlatformOpen && <AddPlatform onClose={() => setIsAddPlatformOpen(false)} companyId={Number(localStorage.getItem("userId"))} />}
      </div>
  );
}
