...
import { getJoinRequests, acceptAstronaut, declineAstronaut } from "../../calling/companyCaller";
...
export default function AstronautRequest() {
  ...
  const handleAccept = (astronautId: number, missionId: number) => {
    acceptAstronaut(astronautId, missionId).then(() => {
      setRequests((prevRequests) => prevRequests.filter((req) => req.userId !== astronautId));
    });
  };
      ...
      <div className="w-full max-w-6xl p-6 overflow-auto" style={{ maxHeight: "calc(100vh - 200px)" }}>
        ...
        <div className="flex flex-col space-y-4">
          {requests.map((astronaut: Astronaut) => (
            <div className="flex flex-col md:flex-row items-center bg-white rounded-lg shadow-md overflow-hidden hover:-translate-y-1 hover:shadow-lg w-full" key={astronaut.userId}>
              ...
              <div className="p-4 flex space-x-2">
                <button onClick={() => handleAccept(astronaut.userId, astronaut.missionId)} className="bg-green-500 text-white px-4 py-2 rounded">
                  Accept
                </button>
                <button onClick={() => handleDecline(astronaut.userId, astronaut.missionId)} className="bg-red-500 text-white px-4 py-2 rounded">
                  Decline
                </button>
              </div>
            </div>
