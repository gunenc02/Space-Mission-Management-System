...
import {
  filterSpaceMissions,
  getSpaceMissions,
} from "../../calling/spaceMissionCaller";
...
export default function SpaceMissions() {
  ...
  const [filteredSpaceMissions, setFilteredSpaceMissions] = useState<
    SpaceMission[]
  >([]);
          ...
          <div className="flex space-x-2">
            {localStorage.getItem("userRole") === "COMPANY" && (
                <button
                    ...
                    className="px-6 py-2 bg-blue-500 text-white rounded-lg shadow hover:bg-blue-600 min-w-[150px] text-sm sm:text-base roll break-keep"
                >
            ...
            <button
                ...
                className="px-6 py-2 bg-green-500 text-white rounded-lg shadow hover:bg-green-600 min-w-[150px] text-sm sm:text-base w-56"
            >
              Filter
            </button>
          </div>
        ...
        <div className="flex flex-col space-y-4">
          {filteredSpaceMissions.map((spaceMission: SpaceMission) => (
              <Link
                  ...
                  key={spaceMission.id}
              >
                <div className="w-full md:w-1/3 h-48 flex-shrink-0"> {/* Ensure the container has a fixed size */}
                  <img
