...
import { CreateHealthRecordProps } from "../../data-types/props";
...
export default function CreateHealthRecord(props: CreateHealthRecordProps) {
  ...
  const registerHandler = function () {
    ...
    const formData = {
      ...
      bloodPressure:
        document.getElementById("tfBloodPressure")?.nodeValue || "",
      vaccinations:
        (document.getElementById("tfVaccinations") as HTMLTextAreaElement)
          ?.value || "",
      notes:
        (document.getElementById("tfNotes") as HTMLTextAreaElement)?.value ||
        "",
      eligible:
        (
          document.querySelector(
            'input[name="eligible"]:checked'
          ) as HTMLInputElement
        )?.value || "",
    };
  ...
  return (
    <div style={{ width: CHR_WIDTH, height: CHR_HEIGHT }}>
      ...
      <h5
        style={{ color: "red", visibility: errVisible ? "visible" : "hidden" }}
      >
